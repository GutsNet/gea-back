...
def recalcular_infestacion(arbol):
    ...
    El nivel se calcula como la suma de los 3 componentes (n1, n2, n3)
    de la escala Hawksworth, con un rango máximo de 7.5.
    """
    ultimo_reporte = (
        arbol.reportes.filter(status_reporte="Validado").order_by("-hora").first()
    )
    if ultimo_reporte:
        total = ultimo_reporte.n1 + ultimo_reporte.n2 + ultimo_reporte.n3
        arbol.nivel_infestacion = round(total, 2)
